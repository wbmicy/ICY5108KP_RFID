...
void NFC_Initiator_PrintMode(tNfcInitiatorState state)
{
    char modStrs[][50] = {
                            "NFC_INITIATOR_TECH_DETECTION",
    ...
    Serial_printf(modStrs[(uint8_t)state],RW_STATUS_DATA);
}
//*****************************************************************************
//
...
void NFC_Initiator_Init(void)
{
	...
	{
		Serial_printf("Initiator In :",RW_STATUS_DATA);//DEP_LAYER_DATA,NFC_INITIATOR_LAYER_DATA);
		NFC_Initiator_PrintMode(eTempNfcState);
		Serial_printf("Initiator Out :",RW_STATUS_DATA);//,DEP_LAYER_DATA,NFC_INITIATOR_LAYER_DATA);
...
//		MCU_delayMillisecond(12); // This is calculated from the requirement but testing with MicroPross showed the delay was excessive.
	...
	if(eTempNfcState != g_sCurrentPollState)
	{
		Serial_printf("Initiator In :",RW_STATUS_DATA);//,NFC_INITIATOR_LAYER_DATA);
		NFC_Initiator_PrintMode(eTempNfcState);
		Serial_printf("Initiator Out :",RW_STATUS_DATA);//,NFC_INITIATOR_LAYER_DATA);
		NFC_Initiator_PrintMode(g_sCurrentPollState);
	}
#endif
